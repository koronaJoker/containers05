...
- **apache2/000-default.conf** – изменили `ServerName` на localhost, поменяли `ServerAdmin` на свой, добавили `DirectoryIndex index.php index.html`.  
- **apache2/apache2.conf** – в конце добавили `ServerName localhost`.  
- **php/php.ini** – включили лог ошибок (`error_log`), поменяли `memory_limit = 128M`, `upload_max_filesize = 128M`, `post_max_size = 128M`, `max_execution_time = 120`.  
- **mariadb/50-server.cnf** – раскомментировали `log_error = /var/log/mysql/error.log`.  
- **wp-config.php** – добавили настройки подключения WordPress к базе данных (имя базы, пользователь, пароль, сервер, префикс таблиц).  

**2. За что отвечает инструкция DirectoryIndex в файле конфигурации apache2?**  
- Указывает, какой файл загружать первым при открытии папки на сайте (например, index.php, index.html)

**3. Зачем нужен файл wp-config.php?**  
- Хранит настройки подключения WordPress к базе данных и другие параметры сайта

**4. За что отвечает параметр post_max_size в файле конфигурации php?**  
- Ограничивает максимальный размер данных, отправляемых через форму на сайте

**5. Какие недостатки есть в созданном образе контейнера?**  
- Большой размер, **все сервисы в одном контейнере**, нет автоматической настройки безопасности, возможна потеря данных без томов
